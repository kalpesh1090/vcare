				...
				<div class="form-group">
					<div class="row">
						<div class="col-lg-6">
							...
							<input type="text"  value="{{$district->name}}" class="form-control" name="name" placeholder="Name">
                            <span  class="name text-danger"></span>

						</div>
            <input type="text" hidden name="id" value="{{$district->id}}">
            <div class="col-lg-6">
							<label>State<span class="text-danger">*</span></label>
                            <select name="state_id" class="custom-select" >
                                 <option value="">Choose</option>
                                 @foreach ($states as $state )
                                 <option {{($state->id==$district->state_id)?'selected="selected"' : ''}}value="{{$state->id}}">{{$state->name}}</option>
                ...
                <div class="text-right">
                    <a href="{{url('district')}}" type="button" id="createBtn" class="btn btn-danger">Cancel</a>
   ...
   jQuery(function($) {
    $(document).on('click', '#createBtn', function(e) {
      ...
      $.ajax({
        ...
        url: "{{ route('district.post_update') }}",
...
$.each(error.errors,function(k,v){
    ...
    if(k=="state_id"){
        ...
        success: function(data) {
          ...
          window.location.replace('{{ route("district.index")}}');
